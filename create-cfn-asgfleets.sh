...
echo "Creating $codestardemo_asgfleets_stackname."
...
elb_sg="$(aws cloudformation describe-stacks --stack-name $codestardemo_securitygroup_stackname --output text --query 'Stacks[0].Outputs[?OutputKey==`ELBSecurityGroup`].OutputValue')"
...
echo "Created $codestardemo_asgfleets_stackname."
