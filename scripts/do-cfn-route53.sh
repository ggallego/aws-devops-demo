...
echo "Creating $awsdevopsdemo_route53_stackname."

codedeploy_elb_beta="$(aws cloudformation describe-stacks --stack-name $awsdevopsdemo_asgfleets_stackname --output text --query 'Stacks[0].Outputs[?OutputKey==`BetaELBId`].OutputValue')"
codedeploy_elb_prod="$(aws cloudformation describe-stacks --stack-name $awsdevopsdemo_asgfleets_stackname --output text --query 'Stacks[0].Outputs[?OutputKey==`ProdELBId`].OutputValue')"
